class BossBaseState {
    ...
    updateState(stateManager) {
        ...
        if(currentTime - BossBaseState.LastAnimationPlayed >= this.IntervalBetweenFrames) {
            this.imageCounter = (this.imageCounter + 1) % this.maxImages;
            stateManager.boss.currentImage = this.images[this.imageCounter];
            
            BossBaseState.LastAnimationPlayed = currentTime;
        }
